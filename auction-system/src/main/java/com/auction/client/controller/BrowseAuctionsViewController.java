 ...
 *
 * Gọi action "LIST_AUCTIONS" tới server (nếu server có hỗ trợ).
 * Nếu server chưa hỗ trợ, sẽ hiển thị bảng rỗng + thông báo.
 */
public class BrowseAuctionsViewController {
    ...
    @FXML private TableColumn<AuctionRow, String> colStart;
    @FXML private TableColumn<AuctionRow, String> colCurrent;
    ...
    @FXML public void handleReload() { loadAuctions(null); }
    ...
    /**
     * Gọi server "LIST_AUCTIONS". Server hiện chưa có action này
     * thì ta vẫn xử lý fallback gracefully.
     */
        ...
        try {
            ...
            for (JsonElement e : arr) {
                ...
                if (filter == null || filter.isEmpty()
                        || r.nameProp.get().toLowerCase().contains(filter)
                        || r.idProp.get().contains(filter)) {
                    data.add(r);
                }
            }
        } catch (Exception ignored) { /* server chưa hỗ trợ – bỏ qua */ }
    }
