...
public class HomeViewController {
    ...
    @FXML private FlowPane quickActions;
    ...
    private void buildQuickActions() {
        ...
        if (Session.get().isBidder()) {
            quickActions.getChildren().add(action("🔍 Xem phiên đấu giá", "btn-primary",
                    main::showBrowseAuctions));
            quickActions.getChildren().add(action("🎯 Vào chi tiết phiên", "btn-warning",
                    main::showAuctionDetail));
            quickActions.getChildren().add(action("📜 Lịch sử bid của tôi", "btn-ghost",
                    main::showMyBids));
        } else if (Session.get().isSeller()) {
            quickActions.getChildren().add(action("➕ Tạo sản phẩm mới", "btn-success",
                    main::showItemForm));
            quickActions.getChildren().add(action("📦 Quản lý sản phẩm", "btn-primary",
                    main::showMyItems));
            quickActions.getChildren().add(action("🚀 Mở phiên đấu giá", "btn-warning",
                    main::showOpenAuction));
            quickActions.getChildren().add(action("🛎 Phiên của tôi", "btn-ghost",
                    main::showMyAuctions));
        }
    }
