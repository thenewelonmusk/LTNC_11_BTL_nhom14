...
/**
 * Controller chính cho cửa sổ duy nhất.
 * Quản lý header, sidebar (theo role) và content area.
 * Các view con sẽ được load vào contentArea bằng phương thức showView*.
 */
public class MainWindowController {
    ...
    @FXML private Label  lblSecBidder, lblSecSeller;
    @FXML private Button navBrowse, navAuctionDetail, navMyBids;
    @FXML private Button navMyItems, navItemForm, navMyAuctions, navOpenAuction;
    ...
    public static MainWindowController get() { return instance; }
    ...
    private void applyAuthState(boolean loggedIn) {
        ...
        if (loggedIn) {
            ...
            // Section labels
            lblSecBidder.setVisible(bidder); lblSecBidder.setManaged(bidder);
            lblSecSeller.setVisible(seller); lblSecSeller.setManaged(seller);

            // Bidder buttons
            for (Button b : new Button[]{navBrowse, navAuctionDetail, navMyBids}) {
                b.setVisible(bidder); b.setManaged(bidder);
            }
            // Seller buttons
            for (Button b : new Button[]{navMyItems, navItemForm, navMyAuctions, navOpenAuction}) {
                b.setVisible(seller); b.setManaged(seller);
            }
        }
    }
    ...
    private void setActive(Button activeBtn) {
        for (Button b : allNavButtons) b.getStyleClass().remove("active");
    ...
    @FXML public void showHome()           { setActive(navHome);          loadView("/fxml/views/HomeView.fxml"); }
    @FXML public void showBrowseAuctions() { setActive(navBrowse);        loadView("/fxml/views/BrowseAuctionsView.fxml"); }
    @FXML public void showAuctionDetail()  { setActive(navAuctionDetail); loadView("/fxml/views/AuctionDetailView.fxml"); }
    @FXML public void showMyBids()         { setActive(navMyBids);        loadView("/fxml/views/MyBidsView.fxml"); }
    @FXML public void showMyItems()        { setActive(navMyItems);       loadView("/fxml/views/MyItemsView.fxml"); }
    @FXML public void showItemForm()       { setActive(navItemForm);      loadView("/fxml/views/ItemFormView.fxml"); }
    @FXML public void showMyAuctions()     { setActive(navMyAuctions);    loadView("/fxml/views/MyAuctionsView.fxml"); }
    @FXML public void showOpenAuction()    { setActive(navOpenAuction);   loadView("/fxml/views/OpenAuctionView.fxml"); }
