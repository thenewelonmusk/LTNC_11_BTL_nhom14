...
public class OpenAuctionViewController {
	...
	@FXML
	private ComboBox<ItemOption> cbItem;
	@FXML
	...
	public void initialize() {
		...
		cbItem.setConverter(new StringConverter<>() {
			@Override
			public String toString(ItemOption o) {
				return o == null ? "" : ("#" + o.id + " · " + o.name);
			}
			@Override
			public ItemOption fromString(String s) {
				return null;
			}
		});
		...
		try {
			...
			for (JsonElement e : arr) {
				...
				opt.startingPrice = o.has("startingPrice") && !o.get("startingPrice").isJsonNull()
						? o.get("startingPrice").getAsDouble()
						: 0;
				itemOptions.add(opt);
			}
	...
	public void handleOpenAuction() {
		...
		double price;
		try {
			price = Double.parseDouble(txtStartPrice.getText().trim());
		} catch (Exception e) {
