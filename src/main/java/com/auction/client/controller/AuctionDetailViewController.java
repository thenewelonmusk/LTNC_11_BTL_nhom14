...
public class AuctionDetailViewController implements NetworkClient.AuctionUpdateListener {
    ...
    @FXML private TableColumn<BidRow, Double> colAmount;
    ...
    private void parseAndPopulateUI(String jsonStr) {
        try {
            ...
            JsonObject auction = root.has("auction") && root.get("auction").isJsonObject()
                    ? root.getAsJsonObject("auction") : new JsonObject();
            JsonObject item = root.has("item") && root.get("item").isJsonObject()
                    ? root.getAsJsonObject("item") : new JsonObject();
            JsonArray bids = root.has("bids") && root.get("bids").isJsonArray()
                    ? root.getAsJsonArray("bids") : new JsonArray();

            if (lblItemName != null) lblItemName.setText(getStr(item, "name", "-"));
            if (lblItemType != null) lblItemType.setText(getStr(item, "type", "-"));
            if (lblItemDesc != null) lblItemDesc.setText(getStr(item, "description", "-"));
            if (lblStart != null) lblStart.setText(formatMoney(getDouble(auction, "startingPrice")));
            if (lblCurrent != null) lblCurrent.setText(formatMoney(getDouble(auction, "currentPrice")));
            if (lblEnd != null) lblEnd.setText(getStr(auction, "endTime", "-"));
            if (lblStatus != null) lblStatus.setText(getStr(auction, "status", "-"));
            if (lblBidResult != null) lblBidResult.setText("");
            ...
            new Thread(() -> {
                ...
                Platform.runLater(() -> {
                    ...
                    try {
                        ...
                        String message = getStr(resObj, "message", success ? "Đặt giá thành công!" : "Đặt giá thất bại.");
    ...
    public static class BidRow {
        ...
        public SimpleStringProperty bidderProperty() { return bidder; }
