...
public class BidServiceImpl implements BidService {
	...
	// Map này được CHIA SẺ với AutoBidServiceImpl để cùng dùng một lock per-auction.
	private final ConcurrentHashMap<Long, ReentrantLock> auctionLocks = new ConcurrentHashMap<>();

	public BidServiceImpl(BidDAO bidDAO, AuctionDAO auctionDAO, AuctionService auctionService, AutoBidService autoBidService) {
		this.bidDAO = bidDAO;
		this.auctionDAO = auctionDAO;
		this.auctionService = auctionService;
		this.autoBidService = autoBidService;

		// Cho AutoBidService biết bảng lock này để cùng đồng bộ
		if (autoBidService instanceof AutoBidServiceImpl) {
			((AutoBidServiceImpl) autoBidService).setSharedLockMap(this.auctionLocks);
		}
	}
	...
	public BidResponse placeBid(BidRequest request, Long bidderId) {
		return placeBid(request, bidderId, null);
	}

	@Override
	public BidResponse placeBid(BidRequest request, Long bidderId, AutoBidService.AutoBidListener listener) {
		...
		lock.lock();
		try {
			// 🌟 FIX QUAN TRỌNG 1: Mở kết nối Database tại đây, ôm trọn cả người thật và Bot
			com.auction.dao.DatabaseConnection.beginTransaction();

			// 1. Xử lý logic đặt giá của người dùng thật (Đã tháo lệnh commit() ra khỏi hàm này)
			BidResponse response = processBid(request, bidderId);

			// 2. Kích hoạt vòng đấu Auto-bid NGAY TRONG CÙNG 1 TRANSACTION
			if (response.isSuccess() && autoBidService != null) {
				...
				try {
					...
					// Chộp lại Transaction giá cao nhất (của Bot vừa đánh)
					BidTransaction highestBid = bidDAO.findHighestBidByAuction(auctionId);

					response = new BidResponse(true, response.getMessage(), highestBid, refreshed.getCurrentPrice());
				} catch (Exception e) {
					System.err.println("[BidService] Lỗi khi làm mới phiên đấu giá: " + e.getMessage());
				}
			}

			// 🌟 FIX QUAN TRỌNG 2: Tất cả đều an toàn thì mới Commit chốt hạ vào DB
			com.auction.dao.DatabaseConnection.commit();
			return response;

		} catch (Exception e) {
			// 🌟 FIX QUAN TRỌNG 3: Bất kỳ ai lỗi (Người/Bot), Rollback sạch sẽ
	...
	@Override
	public AutoBidResponse registerAutoBidWithLock(AutoBidRequest request, AutoBidService.AutoBidListener listener) {
		if (autoBidService == null) {
			return new AutoBidResponse(false, "Auto-bid service không khả dụng.", null);
		}
		if (request == null || request.getAuctionId() == null) {
			return new AutoBidResponse(false, ERROR_INVALID_REQUEST, null);
		}
		...
		lock.lock();
		try {
			// Bao bọc cả thao tác nút Lưu AutoBid bằng Transaction đề phòng Bot đánh luôn
			com.auction.dao.DatabaseConnection.beginTransaction();

			AutoBidResponse response = autoBidService.registerAutoBidAndTrigger(request, listener);

			com.auction.dao.DatabaseConnection.commit();
			return response;
		} catch (Exception e) {
			com.auction.dao.DatabaseConnection.cleanup();
			return new AutoBidResponse(false, ERROR_SAVE_FAILED, null);
		} finally {
			lock.unlock();
		}
	}
	...
	/**
	 * Xử lý bid thật sự (chỉ là luồng logic)
	 * Đã LƯỢC BỎ lệnh đóng/mở Transaction bên trong để gộp chung với Bot.
	 */
