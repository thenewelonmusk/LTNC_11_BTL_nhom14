 ...
 * Thread-safety: - Map registry dùng ConcurrentHashMap. - Vòng đấu auto-bid
 * trên một phiên được đồng bộ qua cùng cơ chế khóa của BidServiceImpl:
 * AutoBidServiceImpl được TRUYỀN VÀO BidServiceImpl, và BidServiceImpl gọi
 * triggerAutoBids() từ trong khối lock của nó. Nhờ vậy, vòng đấu auto-bid và
 * bid thường không bao giờ chạy chồng nhau trên cùng một auctionId -> tránh
 * lost update, double-winner, rollback giá.
 */
public class AutoBidServiceImpl implements AutoBidService {
	 ...
	 *
	 * Thuật toán: - Auto-bid chỉ phản ứng khi đã CÓ AI đó giữ giá (winnerId !=
	 * null). Nếu chưa ai bid, vòng đấu không tự khởi động (đúng nghĩa "auto-bid là
	 * tự động trả thay" - phải có bid của đối thủ trước). - Lặp cho đến khi không
	 * còn ai có thể vượt giá hiện tại: 1. Lọc các auto-bid ACTIVE, KHÔNG phải người
	 * đang giữ giá cao nhất, và còn khả năng trả (maxBid >= currentPrice +
	 * max(MIN_INCREMENT, increment riêng)). 2. Nếu không còn ai -> thoát. 3. Trong
	 * các ứng viên, chọn người có maxBid CAO NHẤT. Nếu nhiều người cùng maxBid cao
	 * nhất: chọn người đăng ký SỚM NHẤT. 4. Tính nextPrice = currentPrice +
	 * increment của người đó. Nếu nextPrice vượt maxBid của họ -> ép xuống maxBid.
	 * 5. Lưu BidTransaction (autoBid=true), cập nhật auction. 6. Tiếp tục vòng lặp.
	 *
	...
	public int triggerAutoBids(Long auctionId) {
		...
		Map<Long, AutoBidEntry> perAuction = registry.get(auctionId);
		if (perAuction == null || perAuction.isEmpty())
			return 0;
	...
	/**
	 * Chọn auto-bid tốt nhất để đặt tiếp theo: - active = true - bidder !=
	 * currentWinner (không tự đè giá của chính mình) - maxBid >= currentPrice +
	 * MIN_INCREMENT (còn khả năng vượt) - sắp xếp: maxBid giảm dần, registeredAt
	 * tăng dần (ưu tiên đăng ký sớm)
	 */
	private AutoBidEntry selectNextAutoBid(Map<Long, AutoBidEntry> perAuction, double currentPrice,
			Long currentWinner) {
		List<AutoBidEntry> candidates = new ArrayList<>();
		for (AutoBidEntry e : perAuction.values()) {
			if (!e.isActive())
				continue;
			if (e.getBidderId().equals(currentWinner))
				continue; // không tự bid chính mình
			if (e.getMaxBid() < currentPrice + MIN_INCREMENT)
				continue; // không đủ tiền
			candidates.add(e);
		}
